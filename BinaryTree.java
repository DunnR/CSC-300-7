...
public class BinaryTree 
{
	...
	{
		int currLeftDepth = this.leftTree.getMaxDepth();
		int currRightDepth = this.rightTree.getMaxDepth();
		return Math.abs(currLeftDepth - currRightDepth);
	}
	
	private void changeDepth(BinaryTree tree)
	{
		int left = -1;
		int right = -1;
			...
			{
				left = tree.leftTree.getMaxDepth();
			}
			if(tree.rightTree != null)
			{
				right = tree.rightTree.getMaxDepth();
			}
